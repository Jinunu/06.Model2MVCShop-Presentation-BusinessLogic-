...
public class ProductTest14 {
	...
	public static void main(String[] args) throws Exception {
		 ...
		 * 
		 * //==> 2. Reader 객체를 이용 xml metadata 에 설정된 각정 정보를 접근, 사용가능한 //==> SqlSession을
		 * 생성하는 SqlSessionFactory instance 생성
		 * 
		 * SqlSessionFactory sqlSessionFactory = new
		 * SqlSessionFactoryBuilder().build(reader); //==>3. SqlSessionFactory 를 통해
		 * autoCommit true 인 SqlSession instance 생성 SqlSession sqlSession =
		...
		Product product = new Product("노트북 이미지", "2000-10-10", 5000, "삼성 놑북", "센스3000");
		 ...
		 * search.setSearchKeyword("마늘소금"); Map<String, Object> map =
		 * productService.getProductList(search);
		 * 
		 * List<Object> list = (List<Object>) map.get("list");
		 * 
		 * for (Object obj : list) { System.out.println("리스트 내용" + obj); } ;
		 * 
		 * System.out.println(productService.getProductList(search)); //
		 * product.setProdName("수정된이름"); // // //
		 */
	}
